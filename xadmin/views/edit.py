...
class ModelFormAdminView(ModelAdminView):
    ...
    @filter_hook
    def get_model_form(self, **kwargs):  # 渲染符合设置的页面
        """
...
class CreateAdminView(ModelFormAdminView):
    ...
    def post_response(self):
        ...
        if "_addanother" in request.POST:
            ...
            # 自定义信息提醒：
            if '/account_manage/' not in self.request.path:
                self.message_user(msg, 'success')  # 注释成功的信息提醒
...
class UpdateAdminView(ModelFormAdminView):
    ...
    def post_response(self):
        ...
        if "_continue" in request.POST:
            ...
            self.message_user(msg, 'success')  # 修改的提示信息
            # Figure out where to redirect. If the user has change permission,
            # redirect to the change-list page for this object. Otherwise,
            # redirect to the admin index.
            if "_redirect" in request.POST:
